...
class JsonRecordProvider:
    ...
    def parse_file(self, file_path: Path | str) -> List[Record]:
        ...
        # Supports:
        # 1. One record as JSON object
        # 2. Many records as JSON array
...
def build_record_from_dict(data: dict) -> Record:
    ...
    record_type = str(normalized_data.get("type", "")).strip().lower()

    if record_type == "news":
        require_fields(normalized_data, ["text", "city"])
        return News(str(normalized_data["text"]), str(normalized_data["city"]))

    if record_type == "privatead":
        require_fields(normalized_data, ["text", "expiration_date"])
        return PrivateAd(
            str(normalized_data["text"]),
            str(normalized_data["expiration_date"]),
        )

    if record_type in {"joke", "jokeoftheday"}:
        require_fields(normalized_data, ["text"])
        return JokeOfTheDay(str(normalized_data["text"]))
