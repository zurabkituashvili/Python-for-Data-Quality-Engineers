...
DEFAULT_JSON_DIR = BASE_DIR / "json_input_records"
DEFAULT_XML_DIR = BASE_DIR / "xml_input_records"
WORD_COUNT_FILE = BASE_DIR / "word-count.csv"
LETTER_STATS_FILE = BASE_DIR / "letter-count.csv"
DB_FILE = BASE_DIR / "records.db"
    ...
    def save_record(self, record: Record) -> bool:
        ...
        try:
            if isinstance(record, News):
                cursor.execute(
                    """
                    INSERT INTO news (text, city, created_at)
                    VALUES (?, ?, ?)
                    """,
                    (
                        record.text,
                ...
                cursor.execute(
                    """
                    INSERT INTO private_ads (text, expiration_date)
                    VALUES (?, ?)
                    """,
                    (
                        record.text,
                ...
                raise ValueError("Unsupported record type for database save")
...
def build_record_from_dict(data: dict) -> Record:
    normalized_data = {str(k).strip().lower(): str(v).strip() for k, v in data.items()}
    record_type = normalized_data.get("type", "").lower()

    if record_type == "news":
        require_fields(normalized_data, ["text", "city"])
        return News(normalized_data["text"], normalized_data["city"])

    if record_type == "privatead":
        require_fields(normalized_data, ["text", "expiration_date"])
        return PrivateAd(
            normalized_data["text"],
            normalized_data["expiration_date"],
        )

    if record_type in {"joke", "jokeoftheday"}:
        require_fields(normalized_data, ["text"])
        return JokeOfTheDay(normalized_data["text"])
...
class FileRecordProvider:
    ...
    def process_file(self, file_path: Path | str) -> bool:
        ...
        try:
            ...
            for record in records:
                inserted = db_manager.save_record(record)
                if inserted:
                    print("Saved to database.")
                else:
                    print("Duplicate found in database. Record was not inserted.")
            db_manager.close()
    ...
    def parse_block(self, block: str) -> Record:
        ...
        return build_record_from_dict(data)


class JsonRecordProvider:
    def __init__(self, default_folder: Path | str = DEFAULT_JSON_DIR) -> None:
        self.default_folder = Path(default_folder)
        self.default_folder.mkdir(parents=True, exist_ok=True)
    ...
    def process_file(self, file_path: Path | str) -> bool:
        ...
        try:
            ...
            os.remove(file_path)
            print(f"Processed and removed JSON file: {file_path}")
            return True
        except Exception as error:
            print(f"Failed to process JSON file '{file_path}': {error}")
            return False
    ...
    def parse_file(self, file_path: Path | str) -> List[Record]:
        ...
        if isinstance(data, list):
            return [build_record_from_dict(item) for item in data]

        raise ValueError("JSON root must be an object or a list of objects")


class XmlRecordProvider:
    def __init__(self, default_folder: Path | str = DEFAULT_XML_DIR) -> None:
        self.default_folder = Path(default_folder)
        self.default_folder.mkdir(parents=True, exist_ok=True)
    ...
    def process(self, path: Optional[str] = None) -> int:
        ...
        files = [target] if target.is_file() else sorted(target.glob("*.xml"))
        processed_files = 0
    ...
    def process_file(self, file_path: Path | str) -> bool:
        ...
        try:
            ...
            os.remove(file_path)
            print(f"Processed and removed XML file: {file_path}")
            return True
        except Exception as error:
            print(f"Failed to process XML file '{file_path}': {error}")
            return False
    ...
    def parse_file(self, file_path: Path | str) -> List[Record]:
        ...
        if root.tag.lower() == "record":
            return [self.parse_record_element(root)]

        if root.tag.lower() == "records":
            records = []
            for record_element in root.findall("record"):
                records.append(self.parse_record_element(record_element))
            return records

        raise ValueError("XML root must be <record> or <records>")

    def parse_record_element(self, element: ET.Element) -> Record:
        data = {}

        for child in element:
            data[child.tag.strip().lower()] = (child.text or "").strip()
...
def save_record_everywhere(record: Record) -> None:
    save_to_file(record.publish())
    ...
    if inserted:
        print("Record added successfully! Saved to database. CSV reports recreated.")
    else:
        print(
            "Record added to file, but duplicate found in database. Database insert skipped."
        )
...
def create_record_from_console() -> Optional[Record]:
    ...
    if choice == "4":
        ...
        path = input(
            f"Enter text file or folder path (press Enter for default folder: {provider.default_folder}): "
        ).strip()
        processed = provider.process(path or None)
        print(f"Successfully processed {processed} text file(s).")
        return None

    if choice == "5":
        provider = JsonRecordProvider()
        path = input(
            f"Enter JSON file or folder path (press Enter for default folder: {provider.default_folder}): "
        ).strip()
        processed = provider.process(path or None)
        print(f"Successfully processed {processed} JSON file(s).")
        return None

    if choice == "6":
        provider = XmlRecordProvider()
        path = input(
            f"Enter XML file or folder path (press Enter for default folder: {provider.default_folder}): "
        ).strip()
        processed = provider.process(path or None)
        print(f"Successfully processed {processed} XML file(s).")
        return None
...
def main() -> None:
    ...
    print(f"Default input folder for JSON files: {DEFAULT_JSON_DIR}")
    print(f"Default input folder for XML files: {DEFAULT_XML_DIR}")
